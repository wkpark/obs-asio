...
public:
			  ...
			  bool force_silent)
	{
		...
		if (!is_audio_planar(out.format)) {
			blog(LOG_ERROR, "that was a goof %i should be %i",
			     out.format, get_planar_format(out.format));
			return false;
		}
		...
		size_t buffer_size =
			(out.frames * sizeof(bytedepth_format(out.format)));
...
public:
	...
	const WinHandle *get_handles() { return receive_signals; }

	WinHandle on_buffer() { return all_received_signal; }

	long get_input_channels() { return input_chs; }
	...
	{
		return (device_source_audio *)circlebuf_data(
			&audio_buffer,
	...
	{
		return (device_source_audio *)circlebuf_data(
			&audio_buffer, index * sizeof(device_source_audio));
	}
	...
	void prep_circle_buffer(DWORD bufpref)
	{
		if (!circle_buffer_prepped) {
			...
			circlebuf_reserve(&audio_buffer,
					  buffer_count *
	...
	void prep_events(BASS_ASIO_INFO &info) { prep_events(info.inputs); }

	void prep_events(long input_chs)
	{
		if (!events_prepped) {
			receive_signals = (WinHandle *)calloc(
				input_chs, sizeof(WinHandle));
			for (int i = 0; i < input_chs; i++) {
				receive_signals[i] = CreateEvent(
	...
	void prep_buffers(BASS_ASIO_INFO &info, audio_format in_format,
			  uint32_t in_samples_per_sec)
	{
		prep_buffers(info.bufpref, info.inputs, in_format,
			     in_samples_per_sec);
	}

	void prep_buffers(uint32_t frames, long in_chs, audio_format format,
			  uint32_t samples_per_sec)
	{
		...
		prep_events(in_chs);
		if (circle_buffer_prepped &&
		    (!buffer_prepped || reallocate_buffer)) {
			this->frames = frames;
			...
			for (int i = 0; i < buffer_count; i++) {
				device_source_audio *_source_audio =
					get_source_audio(i);
				_source_audio->data = (uint8_t **)bzalloc(
					input_chs *
					sizeof(uint8_t *)) /*calloc(input_chs, sizeof(uint8_t*))*/
					;
				for (int j = 0; j < input_chs; j++) {
					if (!buffer_prepped) {
						_source_audio->data
							[j] = (uint8_t *)bzalloc(
							buffer_size) /*calloc(buffer_size, 1)*/
							;
					} else if (reallocate_buffer) {
						uint8_t *tmp = (uint8_t *)realloc(
							_source_audio->data[j],
							buffer_size);
						if (tmp == NULL) {
							buffer_prepped = false;
							all_prepped = false;
	...
	{
		if (!all_prepped) {
			blog(LOG_INFO, "%s device %i is not prepared",
		...
		if (ch_buffer_size > buffer_size) {
			blog(LOG_WARNING,
			     "%s device needs to reallocate memory");
		}
		...
		_source_audio->timestamp = _source_audio->timestamp =
			os_gettime_ns() -
			((_source_audio->frames * NSEC_PER_SEC) /
			 _source_audio->samples_per_sec);
	...
	static DWORD WINAPI capture_thread(void *data)
	{
		...
		asio_data *source =
			pair->asio_listener; //static_cast<asio_data*>(data);
		device_data *device =
			pair->device; //static_cast<device_data*>(data);
		struct obs_audio_info aoi;
		obs_get_audio_info(&aoi);
		...
		thread_name +=
			device->device_info
				.name; //thread_name += " capture thread";
		os_set_thread_name(thread_name.c_str());

		HANDLE signals_1[3] = {device->all_received_signal,
				       device->stop_listening_signal,
				       source->stop_listening_signal};
		HANDLE signals_2[3] = {device->all_received_signal_2,
				       device->stop_listening_signal,
				       source->stop_listening_signal};
		...
		blog(LOG_INFO, "listener for device %lu created: source: %x",
		     device->device_index, source->get_id());

		size_t read_index = device->write_index; //0;
		int waitResult;

		uint64_t buffer_time = ((device->frames * NSEC_PER_SEC) /
					device->samples_per_sec);
		DWORD wait_time = DWORD((buffer_time / 1000000L) * 8);
		blog(LOG_INFO, "device %lu wait_time=%d", device->device_index,
		     wait_time);
		while (source && device) {
			DWORD ms = source->device_timeout ? wait_time
							  : INFINITE;
			waitResult =
				WaitForMultipleObjects(3, signals_1, false, ms);
			waitResult =
				WaitForMultipleObjects(3, signals_2, false, ms);
			...
			if (waitResult == WAIT_OBJECT_0 ||
				...
				if (waitResult == WAIT_TIMEOUT) {
					...
					device->retry_count++;
					if (source->retry_limit != -1 &&
					    device->retry_count >
						    source->retry_limit) {
						// We reached our retry_limit :(
						blog(LOG_INFO,
						     "%s closing due to timeout, device %lu WAIT_TIMEOUT retry_count=%u retry_limit=%u",
						     thread_name.c_str(),
						     device->device_index,
						     device->retry_count,
						     source->retry_limit);
				...
				while (read_index != device->write_index) {
					device_source_audio *in =
						device->get_source_audio(
							read_index); //device->get_writeable_source_audio();
					source->render_audio(
						in, route,
						waitResult == WAIT_TIMEOUT);
					read_index++;
				...
				if (source->device_index !=
				    device->device_index) {
					blog(LOG_INFO,
					     "source device index %lu is not device index %lu",
					     source->device_index,
					     device->device_index);
					blog(LOG_INFO, "%s closing",
					     thread_name.c_str());
					delete pair;
					return 0;
				} else if (!source->isASIOActive) {
					blog(LOG_INFO,
					     "%x indicated it wanted to disconnect",
					     source->get_id());
					blog(LOG_INFO, "%s closing",
					     thread_name.c_str());
					delete pair;
				...
				blog(LOG_INFO,
				     "device %l indicated it wanted to disconnect",
				     device->device_index);
				blog(LOG_INFO, "%s closing",
				     thread_name.c_str());
				delete pair;
				...
				blog(LOG_INFO,
				     "a mutex for %s was abandoned while listening to",
				     thread_name.c_str(), device->device_index);
				blog(LOG_INFO, "%s closing",
				     thread_name.c_str());
				delete pair;
				return 0;
				...
				blog(LOG_INFO,
				     "listener thread wait %lu failed with 0x%x",
				     device->device_index, GetLastError());
				blog(LOG_INFO, "%s closing",
				     thread_name.c_str());
				delete pair;
				return 0;
			} else {
				blog(LOG_INFO, "unexpected wait result = %i",
				     waitResult);
				blog(LOG_INFO, "%s closing",
				     thread_name.c_str());
				delete pair;
	...
	void add_listener(asio_data *listener)
	{
		...
		blog(LOG_INFO,
		     "disconnecting any previous connections (source_id: %x)",
		     listener->get_id());
		listener->disconnect();
		//CloseHandle(listener->captureThread);
		blog(LOG_INFO, "adding listener for %lu (source: %lu)",
...
uint8_t getDeviceCount()
{
	...
	for (a = 0; BASS_ASIO_GetDeviceInfo(a, &info); a++) {
		blog(LOG_INFO, "device index is : %i and name is : %s", a,
		     info.name);
		count++;
	}
...
void fill_out_devices(obs_property_t *list)
{
	...
	for (i = 0; BASS_ASIO_GetDeviceInfo(i, &devinfo); i++) {
		...
		if (!ret) {
			...
			if (!ret)
				blog(LOG_ERROR,
				     "Should never have reached here, wtf uh oh.\n");
		}
	}
}
				       ...
				       obs_data_t *settings)
{
	...
	if (!ret) {
		blog(LOG_ERROR,
		     "Unable to retrieve info on the current driver \n"
...
static bool fill_out_sample_rates(obs_properties_t *props, obs_property_t *list,
				  obs_data_t *settings)
{
	BASS_ASIO_DEVICEINFO devinfo;
	int index = BASS_ASIO_GetDevice();
	...
	if (ret2) {
		...
		blog(LOG_INFO,
		     "Device loaded does not support 44100 Hz sample rate\n");
	}
	if (ret)
	...
	if (ret2) {
		...
		blog(LOG_INFO,
		     "Device loaded does not support 48000 Hz sample rate\n");
	}
	return true;
}

//create list of supported audio formats
static bool fill_out_bit_depths(obs_properties_t *props, obs_property_t *list,
				obs_data_t *settings)
{
	BASS_ASIO_DEVICEINFO devinfo;
	int index = BASS_ASIO_GetDevice();
	...
	if (!ret2) {
		blog(LOG_ERROR,
		     "Unable to retrieve channel info on the current driver \n"
		     "error number is : %i \n; check BASS_ASIO_ErrorGetCode\n",
		     BASS_ASIO_ErrorGetCode());
	}
	...
	if (ret && ret2) {
		if (channelInfo.format == BASS_ASIO_FORMAT_16BIT) {
			obs_property_list_add_int(list, "16 bit (native)",
						  AUDIO_FORMAT_16BIT);
			obs_property_list_add_int(list, "32 bit",
						  AUDIO_FORMAT_32BIT);
			obs_property_list_add_int(list, "32 bit float",
						  AUDIO_FORMAT_FLOAT);
		} else if (channelInfo.format == BASS_ASIO_FORMAT_32BIT) {
			obs_property_list_add_int(list, "16 bit",
						  AUDIO_FORMAT_16BIT);
			obs_property_list_add_int(list, "32 bit (native)",
						  AUDIO_FORMAT_32BIT);
			obs_property_list_add_int(list, "32 bit float",
						  AUDIO_FORMAT_FLOAT);
			...
			obs_property_list_add_int(list, "32 bit",
						  AUDIO_FORMAT_32BIT);
			obs_property_list_add_int(list, "32 bit float (native)",
						  AUDIO_FORMAT_FLOAT);
		} else {
			blog(LOG_ERROR,
			     "Your device uses unsupported bit depth.\n"
...
static bool fill_out_buffer_sizes(obs_properties_t *props, obs_property_t *list,
				  obs_data_t *settings)
{
	BASS_ASIO_INFO info;
	bool ret = BASS_ASIO_GetInfo(&info);
	...
	if (ret) {
		if (info.bufgran == -1) {
			...
			while (gran_buffer <= info.bufmax) {
				int n = snprintf(NULL, 0, "%llu%s", gran_buffer,
						 (gran_buffer == info.bufpref
							  ? " (preferred)"
							  : ""));
				if (n <= 0) {
				...
				char *buf =
					(char *)bmalloc((n + 1) * sizeof(char));
				if (!buf) {
					//problem...continuing on the loop
					gran_buffer *= 2;
					continue;
				}
				int c = snprintf(buf, n + 1, "%llu%s",
						 gran_buffer,
						 (gran_buffer == info.bufpref
							  ? " (preferred)"
							  : ""));
				buf[n] = '\0';
				obs_property_list_add_int(list, buf,
							  gran_buffer);
				bfree(buf);
				gran_buffer *= 2;
			}
			...
			int c = snprintf(buf, n + 1, "%llu%s", gran_buffer,
					 (gran_buffer == info.bufpref
						  ? " (preferred)"
						  : ""));
			buf[n] = '\0';
...
static bool asio_device_changed(obs_properties_t *props, obs_property_t *list,
				obs_data_t *settings)
{
	...
	if (!itemFound) {
		...
		if (!ret) {
			...
			} else if (BASS_ASIO_ErrorGetCode() ==
				   BASS_ERROR_DEVICE) {
			...
			obs_property_set_modified_callback(
				sample_rate, fill_out_sample_rates);
			obs_property_set_modified_callback(bit_depth,
							   fill_out_bit_depths);
			obs_property_set_modified_callback(
				buffer_size, fill_out_buffer_sizes);
		}
	}
	// get channel number from output speaker layout set by obs
...
int mix(uint8_t *inputBuffer, obs_source_audio *out, size_t bytes_per_ch,
	int route[], unsigned int recorded_device_chs = UINT_MAX)
{
	DWORD recorded_channels = get_obs_output_channels();
	short j = 0;
...
DWORD CALLBACK create_asio_buffer(BOOL input, DWORD channel, void *buffer,
				  DWORD BufSize, void *device_ptr)
{
	BASS_ASIO_INFO info;
	bool ret = BASS_ASIO_GetInfo(&info);
...
void CALLBACK asio_device_setting_changed(DWORD notify, void *device_ptr)
{
	...
	if (ret) {
		...
		case BASS_ASIO_NOTIFY_RATE:
			...
			blog(LOG_WARNING, "device %l changed sample rate to %f",
			     device->device_index, new_sample_rate);
			...
			if (!ret) {
				...
				case BASS_ERROR_INIT:
					blog(LOG_ERROR,
					     "Error: Bass asio not initialized.\n");
					break;
				case BASS_ERROR_ALREADY:
					blog(LOG_ERROR,
				...
				case BASS_ERROR_NOCHAN:
					blog(LOG_ERROR,
					     "Error: channels have not been enabled so can not start\n");
					break;
				case BASS_ERROR_UNKNOWN:
				default:
					blog(LOG_ERROR,
					     "ASIO init: Unknown error when trying to start the device\n");
					break;
				}
			}
		...
		case BASS_ASIO_NOTIFY_RESET:
			blog(LOG_WARNING, "device %l requested a reset",
			     device->device_index);
			// Reset ?
...
void asio_init(struct asio_data *data)
{
	...
	if (!BASS_ASIO_IsStarted()) {
		...
		if (obs_optimal_format == asio_native_format) {
			...
			ret = BASS_ASIO_ChannelSetFormat(true, 0,
							 asio_native_format);
			if (!ret) {
				blog(LOG_ERROR,
				     "ASIO: unable to use native format\n"
				     "error number: %i \n; check BASS_ASIO_ErrorGetCode\n",
				     BASS_ASIO_ErrorGetCode());
				return;
			}
			...
			if (!ret) {
				blog(LOG_ERROR,
				     "ASIO: unable to use optimal format (float)\n"
				     "error number: %i \n; check BASS_ASIO_ErrorGetCode\n",
				     BASS_ASIO_ErrorGetCode());

				ret = BASS_ASIO_ChannelSetFormat(
					true, 0, asio_native_format);
				if (!ret) {
					blog(LOG_ERROR,
					     "ASIO: unable to use native format\n"
					     "error number: %i \n; check BASS_ASIO_ErrorGetCode\n",
		...
		audio_format format = get_planar_format(
			asio_to_obs_audio_format(selected_format));

		ret = BASS_ASIO_SetNotify(asio_device_setting_changed,
					  device_list[device_index]);

		// enable all chs and link to callback w/ the device buffer class
		ret = BASS_ASIO_ChannelEnable(true, 0, &create_asio_buffer,
					      device_list[device_index]); //data
		...
		device_list[device_index]->prep_buffers(
			info.bufpref, info.inputs, format, checkrate);
...
void asio_update(void *vptr, obs_data_t *settings)
{
	...
	if (device != NULL && device[0] != '\0') {
		...
		if (!ret) {
			...
			blog(LOG_INFO,
			     "Device %i was successfully initialized\n",
			     device_index);
		...
		if (strcmp(device, devinfo.name) != 0) {
			blog(LOG_ERROR,
			     "Device loaded is not the one in settings\n");
			goto fail;
		}
		...
		for (unsigned int i = 0; i < recorded_channels; i++) {
			...
			route[i] = (int)obs_data_get_int(settings,
							 route_str.c_str());
		...
		data->device_timeout =
			obs_data_get_bool(settings, "device may timeout");
...
void asio_get_defaults(obs_data_t *settings)
{
	...
	for (unsigned int i = 0; i < recorded_channels; i++) {
		std::string name = "route " + std::to_string(i);
		obs_data_set_default_int(settings, name.c_str(),
					 -1); // default is muted channels
	}
}
...
obs_properties_t *asio_get_properties(void *unused)
{
	...
	rate = obs_properties_add_list(props, "sample rate",
				       ...
				       OBS_COMBO_TYPE_LIST,
				       OBS_COMBO_FORMAT_INT);
	std::string rate_descr =
		"Sample rate : number of samples per channel in one second.\n";
	obs_property_set_long_description(rate, rate_descr.c_str());

	bit_depth = obs_properties_add_list(props, "bit depth", TEXT_BITDEPTH,
					    OBS_COMBO_TYPE_LIST,
					    OBS_COMBO_FORMAT_INT);
	std::string bit_descr =
		"Bit depth : size of a sample in bits and format.\n"
	...
	std::string buffer_descr =
		"Buffer : number of samples in a single frame.\n"
	...
	device_timeout =
		obs_properties_add_bool(props, "device may timeout",
					obs_module_text("DeviceMayTimeout"));
	std::string device_timeout_descr =
		"Device May Timeout : Check this if your device has issues starting up / disconnecting. (May solve 'Max audio buffering reached' errors).\n";
	obs_property_set_long_description(device_timeout,
					  device_timeout_descr.c_str());
	obs_property_set_modified_callback(device_timeout,
					   device_timeout_changed);

	retry_limit = obs_properties_add_int(props, "retry limit",
					     obs_module_text("RetryLimit"), -1,
					     65536, 1);
	std::string retry_limit_descr =
		"Retry limit : Maximum retries after device timeouts. -1 = infite retries\n";
	obs_property_set_long_description(retry_limit,
					  retry_limit_descr.c_str());
	obs_property_set_enabled(retry_limit, false);

	console = obs_properties_add_button(
		props, "console", obs_module_text("ASIO driver control panel"),
		DeviceControlPanel);
	std::string console_descr =
		"Make sure your settings in the Driver Control Panel\n"
